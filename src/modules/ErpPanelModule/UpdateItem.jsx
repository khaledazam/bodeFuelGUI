...
export default function UpdateItem({ config, UpdateForm }) {
    ...
    if (fieldsValue) {
      ...
      if (fieldsValue.items) {
        dataToUpdate.items = fieldsValue.items.map((item) => {
          ...
          return {
            ...
            quantity: qty,
            price: price,
            total: calculate.multiply(qty, price),
          };
        });
      }
      ...
      numericFields.forEach(field => {
        if (dataToUpdate[field] !== undefined) {
          dataToUpdate[field] = toNumber(dataToUpdate[field]);
        }
      });
    }
  ...
  useEffect(() => {
    if (current) {
      ...
      // IMPORTANT: prevent overwrite if user already touched/started editing the form
